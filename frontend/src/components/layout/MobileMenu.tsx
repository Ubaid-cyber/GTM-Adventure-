...
import { Menu, X, ChevronRight, Compass, Shield, Heart, User, LogOut } from 'lucide-react';
...
export default function MobileMenu({ session }: MobileMenuProps) {
  ...
  const navLinks = [
    ...
    { href: '/dashboard/bookings', label: 'My Expeditions', icon: Shield, protected: true },
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                <div className="space-y-2">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-4">Expedition Menu</h3>
                  {navLinks.map((link) => {
                    ...
                    return (
                      <Link 
                        ...
                        href={link.href}
                        onClick={toggleMenu}
                        className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary/20 hover:bg-primary/5 transition-all group"
                      >
                        <div className="flex items-center gap-4">
                           <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-primary group-hover:border-primary/20 transition-all shadow-sm">
                              <Icon size={20} />
                           </div>
              ...
              {isLoggedIn && (
                <div className="p-6 bg-slate-50 border-t border-slate-200">
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-white border border-rose-100 text-rose-500 font-black text-xs uppercase tracking-widest hover:bg-rose-50 transition-all"
                  >
                    <LogOut size={16} />
                    Sign Out
                  </button>
